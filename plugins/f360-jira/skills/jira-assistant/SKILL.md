...
1. **Check workspace rules**: Look for Jira configuration in `.cursor/rules/jira-config.mdc` (workspace)
2. **If not found, check user rules**: Look in `~/.cursor/rules/jira-config.mdc` (pasta do usuário)
3. **If not found**: Use MCP search tools to discover available projects
4. **If still unclear**: Ask user to specify project key
5. **Use detected values** for all Jira operations in this conversation

### Configuration Detection Workflow

When you activate this skill:

1. Check workspace for `.cursor/rules/jira-config.mdc` with Jira configuration
2. **If not found**, check user folder: `~/.cursor/rules/jira-config.mdc` (no Windows: `%USERPROFILE%\.cursor\rules\jira-config.mdc`)
3. If found in either location, extract and use: Project Key, Cloud ID, URL, Board URL
4. If not found in either place:
...
**Note for skill users:** To configure this skill, create `~/.cursor/rules/jira-config.mdc` in your user folder (applies to all workspaces), or `.cursor/rules/jira-config.mdc` in the project for workspace-specific config.
...
- **Use detected configuration values** from workspace rules, user folder (`~/.cursor/rules/jira-config.mdc`), or user input
...
- **Use detected configuration** - Read from `.cursor/rules/jira-config.mdc` (workspace), then `~/.cursor/rules/jira-config.mdc` (user folder), or ask user
